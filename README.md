...
### **부가 기능**
1. **로그아웃**  
   - 로그인된 사용자의 세션을 종료하고 `/users/landing`으로 이동
...
3. **접근 권한 관리**  
   ...
   - **예시**:  
     - 로그아웃 상태에서 `/users/userinfo` 접근 불가 → `/users/landing`으로 이동  
     - 로그인 상태에서 `/users/landing` 접근 불가 → `/users/userinfo`로 이동  
...
2. **`POST` 요청**:
   ...
   - 계정 생성 후:
     - 사용자 자동 로그인 처리
     - 사용자 정보 페이지(`/users/userinfo/`)로 이동

#### **소셜 로그인 (OAuth2.0)**
1. **소셜 로그인 요청**:
   - 사용자가 카카오/네이버 로그인 버튼 클릭
   - 해당 소셜 플랫폼의 인증 페이지로 이동
...
3. **사용자 데이터 처리**:
   ...
   - 가져온 데이터를 기반으로 `populate_user` 메서드가 실행:
     - 사용자의 데이터(닉네임, 프로필 이미지, 이메일, 별명, 생일) 저장
     - 기존 동일 이메일의 계정이 존재할 경우
       - 계정 연결
       - 로그인 후 사용자 정보 페이지(`/users/userinfo/`)로 이동
...
1. **로그아웃 상태**:
   - `/users/userinfo/` 접근 시 로그인 페이지(`/users/landing/`)로 이동

2. **로그인 상태**:
   - `/users/landing/` 접근 시 사용자 정보 페이지(`/users/userinfo/`)로 이동
...
- 유효한 데이터를 입력받으면 새 사용자 계정을 생성하고 자동으로 로그인하게 됩니다.
...
def signup_view(request):
    if request.method == 'POST':
        ...
        form = SignupForm() # GET 요청시 비어있는 폼 제공
    context = {
        'form': form
    }
...
- 로그인 상태를 기반으로 특정 페이지 접근 제한.  
  - 예: 로그인 상태에서 `/users/landing` 접근 시 `/user/info`로 이동  
  - 로그아웃 상태에서 `/users/userinfo` 접근 시 `/user/landing`으로 이동
